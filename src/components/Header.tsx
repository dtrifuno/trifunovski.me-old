...
const NavLink: React.FC<LinkProps> = ({ to, children }) => {
  return (
    <li>
      <Link
        className="px-2 py-1 font-medium text-xl text-gray-600 lowercase hover:text-orange-600"
        activeClassName="text-orange-600 border-b-2 border-orange-600"
        to={to}
      >
        {children}
      </Link>
  ...
  return (
    <li>
      <Link
        ...
        activeClassName="text-white hover:text-white bg-orange-600 hover:bg-orange-600"
        to={to}
      >
        {children}
      </Link>
...
const MenuButton: React.FC<MenuButtonProps> = ({ onClick }) => {
  return (
    <button
      className="inline-flex items-center justify-center p-2 rounded-md hover:text-white hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white"
      aria-expanded="false"
      onClick={onClick}
    >
    ...
    <header className="border-b-2 border-orange-600">
      <div className="mx-auto">
        ...
        <nav className={clsx({ 'sm:hidden': true, hidden: isMenuHidden })}>
          <ul className="list-none px-2 pt-2 pb-3 space-y-1 border-t-2 border-orange-600">
            {links.map(link => (
